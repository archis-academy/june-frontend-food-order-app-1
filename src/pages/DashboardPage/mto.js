      ...
      if (index !== -1) {
        ...
        if (
          ...
          const maskFillRectY =
            50 -
            innerCircleRadius +
            2 * innerCircleRadius * (1 - completionPercentageForInner / 100);
          ...
          maskFillRect.setAttribute(
            "height",
            2 * innerCircleRadius * (completionPercentageForInner / 100)
          );
        } else {
          const currentRadius =
            50 -
...
$(document).ready(function () {
  ...
  function updateChart(data) {
      $progressContainer.radialMultiProgress("setOption", "data", data);
      updateText(data);
  }

  function updateText(data) {
      // Clear previous text
      $(".mto__chart-info-text-under h3").each(function () {
          $(this).text("");
      });

      // Update text based on data
      data.forEach(function (item, index) {
          $(".mto__chart-info-text-under h3").eq(index).text(item.value + " customers");
      });
  }
});
